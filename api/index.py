...
def init_analyzer():
    """Initialize GitHub model handler for personality analysis"""
    global analyzer
    if analyzer is None:
        try:
            # Use GitHub releases model handler
            from model_handler import model_handler
            analyzer = model_handler
            logger.info("✅ GitHub model handler initialized successfully")
        except Exception as e:
            logger.error(f"❌ Error initializing GitHub model handler: {e}")
            raise Exception(f"Model handler initialization failed: {str(e)}")
    return analyzer
